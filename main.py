...
api_id = 25004026
api_hash = 'c099cac9cb4e8e15398db4dd40342568'
bot_token = "6220038329:AAHyutBwfWIct0yhJ2t5HPoSa0KDDzlfImk"
                        ...
                        if r.title == lis[int(a)]:
                            ...
                            if mgres.text == 1 or mgres.text == "1":
                                for c in chapters:
                                    ...
                                    print("[MM] [{}] {} @Manga_Mix.pdf".format(z, lis[int(a)]))
                                    try:
                                        chapter_path = c.download(r"./Chapter {} - {}.pdf".format(c.chapter, lis[int(a)]))
                                        await client.send_file(event.chat_id, file = chapter_path, caption = "@Manga_Mix", thumb = r"thumb.png")
                                ...
                                for c in chapters:
                                    if maxint == -1:
                                        if int(c.chapter) >= minint:
                                            ...
                                            try:
                                                chapter_path = c.download(r"./Chapter {} - {}.pdf".format(c.chapter, lis[int(a)]))
                                                await client.send_file(event.chat_id, file = chapter_path, caption = "@manhwa_lord_manhua_manga", thumb = r"thumb.png")
                                                os.remove(chapter_path)
                                                z +=1
                                            except Exception as e:
                                                error = "COULD NOT DOWNLOAD CHAPTER NO. {}. PLEASE DOWNLOAD IT MANUALLY".format(c.chapter)
                                ...
                                await conv.send_message("Enter which chapter you have to download:\n(For example if you have to download 15. Enter 15)")
